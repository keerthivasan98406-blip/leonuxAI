...
Always format business information with numbered points and clear line breaks for readability.${userContext}`
  ...
  const response = await fetch(`${API_URL}/chat`, {
    ...
    body: JSON.stringify({
      model: imageBase64 ? 'google/gemma-2-9b-it:free' : 'openrouter/hunter-alpha',
      messages: messages,
      stream: true
    })
  });
  ...
  console.log('🎯 Model used:', imageBase64 ? 'google/gemma-2-9b-it:free (vision)' : 'openrouter/hunter-alpha (text)');
