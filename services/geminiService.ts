...
Always format business information with numbered points and clear line breaks for readability.${userContext}`
  ...
  const response = await fetch(`${API_URL}/chat`, {
    ...
    body: JSON.stringify({
      model: imageBase64 ? 'mistralai/mistral-small-3.1-24b-instruct:free' : 'openrouter/hunter-alpha',
      messages: messages,
      stream: true
    })
  });
  ...
  console.log('🎯 Model used:', imageBase64 ? 'mistralai/mistral-small-3.1-24b-instruct:free (vision)' : 'openrouter/hunter-alpha (text)');
